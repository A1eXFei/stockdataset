...
class StockBasicInfoDaoImpl:
    ...
    def get_stock_codes(self):
        ...
        logging.debug("列表长度:" + str(len(codes)))
        return codes

    def add(self, sse_file_path, szse_file_path):
        stock_list = []
        logging.info("读取上交所文件...")
        ...
        logging.info("股票的总数为：" + str(len(stock_list)))
        sess = sessionmaker(bind=dbu.get_engine())()
        for each in stock_list:
            logging.debug("处理股票代码：" + each.code)
            try:
                _ = sess.query(Stock).filter_by(code=each.code).one()
                logging.debug("股票代码" + each.code + "已存在")
                pass
            except sqlalchemy.orm.exc.NoResultFound as _:
                sess.add(each)
                logging.debug("股票代码" + each.code + "为新股，将存入数据库")
